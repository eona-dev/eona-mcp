...
The MCP bootstrap installs the MCP surface and provisions `eona-cli` as a sibling runtime when `~/.eona/eona-cli/bin/eona` is missing. It leaves an existing CLI runtime in place so `eona-cli` can self-upgrade independently; use `--repair-cli` to run the CLI bootstrap anyway.
